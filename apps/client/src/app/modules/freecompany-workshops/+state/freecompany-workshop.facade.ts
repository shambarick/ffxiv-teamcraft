...
export class FreecompanyWorkshopFacade {
  ...
  public getSubmarineBuild(submarine: Submarine) {
    return {
      hull: this.lazyData.data.submarineParts[submarine.hullId],
      stern: this.lazyData.data.submarineParts[submarine.sternId],
      bow: this.lazyData.data.submarineParts[submarine.bowId],
      bridge: this.lazyData.data.submarineParts[submarine.bridgeId]
    };
  }

  public getSubmarineStats(submarine: Submarine): VesselStats {
    const hull = this.lazyData.data.submarineParts[submarine.hullId];
    const stern = this.lazyData.data.submarineParts[submarine.sternId];
    const bow = this.lazyData.data.submarineParts[submarine.bowId];
    const bridge = this.lazyData.data.submarineParts[submarine.bridgeId];
    return {
